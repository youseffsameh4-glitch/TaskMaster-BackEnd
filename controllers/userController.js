...
console.log("Identifier received:", identifier);
  ...
  if (!oldPassword || !newPassword) {
    return res.status(400).json({ message: "برجاء كتابة الباسورد القديم والجديد معاً!" });
  }
...
const signup = catchAsync(async (req, res, next) => {
    const { email, name, password } = req.body;
  // make sure that the user has provided email, name, password
  //hash the password
    const hashedPassword = await bcrypt.hash(password, 10);
    ...
    //delete the password from appearing in the response
    delete newUser.password;
    res.json({ message: "user created successfully", user: newUser });
  });

//7. login user and send JWT token
